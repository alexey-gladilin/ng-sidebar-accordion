...
export class AppComponent implements AfterViewInit {
  ...
  onSidebarOpenedChange(e: SidebarComponent): void {
    this.txtSidebarOpenedChanged.nativeElement.value += `position: ${
      e.position
    }, index: ${this.accordion.getSidebarIndex(e)} (${
      e.opened ? 'open' : 'close'
    })\r\n`;
  }

  onSidebarResizableBegin(e: Position): void {
    this.txtSidebarOpenedChanged.nativeElement.value += `sidebar begin of resizable: ${e}\r\n`;
  }

  onSidebarResizableEnd(e: Position): void {
    this.txtSidebarOpenedChanged.nativeElement.value += `sidebar end of resizable: ${e}\r\n`;
  }

  getMode(position: string): string {
    const sidebarSettings =
      this.accordion && this.accordion.sideBarSettingsList
        ? this.accordion.sideBarSettingsList.find(
  ...
  onApplyClick() {
    const sidebarSettings = this.accordion.sideBarSettingsList.find(
      (s) => s.position === this.slSectionModeElmPosition.nativeElement.value
    );
  ...
  onPinnedClick(e: MouseEvent, sidebar) {
    e.stopPropagation();
