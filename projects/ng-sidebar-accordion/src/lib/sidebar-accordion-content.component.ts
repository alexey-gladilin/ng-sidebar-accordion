...
export class SidebarAccordionContentComponent {
  @HostBinding('class.ng-sidebar-accordion-content')
  classNameSidebarAccordionContent = true;
  @Input() @HostBinding('class') className: string;

  constructor(@Optional() private _container: SidebarAccordionComponent) {
    if (!this._container) {
      throw new Error(
        '<ng-sidebar-accordion-content> must be inside a <ng-sidebar-accordion></ng-sidebar-accordion>.'
      );
    }
  }
}
